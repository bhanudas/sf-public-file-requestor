...
export default class DocumentReviewPanel extends NavigationMixin(LightningElement) {
    @api recordId;
    ...
    wiredDetails(result) {
        ...
        if (result.data) {
            ...
            this.error = result.error.body?.message || 'Failed to load request details';
        }
    }
    ...
    get canCommit() {
        return this.files.some(f => f.reviewStatus === 'Approved') &&
    ...
    get statusClass() {
        ...
        if (status === 'Rejected' || status === 'Expired') return 'slds-badge_error';
        if (status === 'Files_Received' || status === 'Under_Review') return 'slds-badge_warning';
        return '';
    }
    ...
    async handleApproveFile(event) {
        ...
        try {
            ...
            this.showToast('Error', error.body?.message || 'Failed to approve file', 'error');
        }
    }
    ...
    async confirmReject() {
        ...
        try {
            ...
            this.showToast('Error', error.body?.message || 'Failed to reject file', 'error');
        }
    }
    ...
    async handleApproveAll() {
        ...
        for (const file of pendingFiles) {
            try {
                await approveFile({ contentVersionId: file.id });
            } catch (error) {
    ...
    async handleCommit() {
        ...
        try {
            ...
            this.showToast('Error', error.body?.message || 'Failed to commit files', 'error');
        } finally {
